...
console.log(data)
	...
	if(state == 'activity') {
		str =  (
			...
			([null,''].includes(data.courseClassificationId) && '积分分类') ||
			([null,''].includes(data.courseClassificationListTwoId) && '二级分类') ||
			(data.courseId == '' && '课程') ||
			...
			if(str == 'ok') {
				...
				str = (
					...
					(data.activityTag && data.activityTag.length > 10 && '活动标签长度不能大于10') ||
					(data.activityPlaceName && data.activityPlaceName.length > 10 && '地点详细描述长度不能大于10') ||
