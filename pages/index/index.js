...
Page({
        ...
        this.setData({
          newList: res.data.map((item) => ({
            ...
            link: item.link.replace(
              "tw.hnust.edu.cn",
              "secondclass.ticknet.hnust.cn/new"
            ), //绕过小程序webview验证
          })),
        });
      })
  ...
  async onLoad() {
    ...
    request({
      ...
      if (
        value.data != wx.getStorageSync("classificationLastTime") ||
        wx.getStorageSync("classificationLastTime") == null
      ) {
        //时间戳不同
  ...
  onPullDownRefresh: function () {
    Promise.all([this.getNews(), this.getActivity()]).then((value) => {
