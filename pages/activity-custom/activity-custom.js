...
Page({
  ...
  sureAddHuman(e) {
    ...
    if (index != -1) {
      ...
      this.data.recordList.unshift({
        //添加记录
        id: e.currentTarget.dataset.id,
        name: e.currentTarget.dataset.name,
      });
    }
    if (this.data.recordList.length > 10) {
      //记录数超限
  ...
  ChooseDeptCheckbox(e) {
    ...
    this.setData({
      deptList: items,
      "postData.enrollRange": this.data.deptList
  ...
  ChooseRangeCheckbox(e) {
    ...
    this.setData({
      range: items,
      "postData.enrollGrade": this.data.range
        .filter((item) => item.checked)
        .map((item) => item.name)
        .join("，"),
    });
  ...
  courseClassificationChange(e) {
    ...
    console.log(
      e.detail,
      this.data.nowYear,
      this.data.postData.trainingProgramId,
      "classiftaion"
    );
    request({
  ...
  onLoad: function (options) {
    ...
    this.setData({
      dict_admissionWay: wx
        .getStorageSync("dict_admissionWay")
        .map((item) => ({
          dictValue: item.dictValue,
          dictLabel: item.dictLabel,
        })),
    ...
    this.setData({
      ...
      dict_evaluate_scheme: wx
        .getStorageSync("dict_evaluate_scheme")
        .map((item) => ({
          dictValue: item.dictValue,
          dictLabel: item.dictLabel,
        })),
      courseClassificationList: filterTwoLayer(
        wx.getStorageSync("courseClassificationList")
      ),
    });
    let temp = [];
    temp.push(this.data.courseClassificationList.map((item) => item.name));
    temp.push(
      this.data.courseClassificationList[0]?.children?.map((item) => item.name)
    );
    this.setData({
      multiArray: temp,
      multiCourseIndex: 0,
    ...
    if (options.aid) {
      ...
      request({
        ...
        let index_first = this.data.courseClassificationList.findIndex(
          (item) => item.id == path[0]
        );
        let index_second = this.data.courseClassificationList[
          index_first
        ].children.findIndex((item) => item.id == path[1]);
        this.data.multiArray[1] = this.data.courseClassificationList[
          index_first
